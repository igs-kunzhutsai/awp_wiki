...
**AWP Backend SDK 的程式碼知識庫，主要讀者是 AI agent。**
它回答的是：這個 SDK 怎麼用、為什麼這樣設計、有什麼不能踩的雷。

事實基準是 `../AWP_Backend/AWP_BACKEND_SYSTEM/`。所有陳述都要能回到那份 source code 驗證。

### 分兩區，責任不同

| | 維護區 | 素材區 `raw/` |
|---|---|---|
| 內容 | 模組行為、串接流程、code 規範、架構決策 | 法規原文、協定通識、遊戲端的事、我們的舊版 |
| code 改了 | **要跟著改** | 不用 |
| 正確性 | **保證，對不上算 bug** | 不保證 |
| 進導覽 | 會 | **不會**，也不算孤島 |
| 推 Outline | 會 | 不推（本來就是從那匯出的）|

**判準只有一句：AWP 的 code 改了，這頁要不要跟著改？**

「不維護」不等於「不存在」—— 素材還在 repo 裡查得到，只是我們不負責它的正確性。
...
|---|---|
| 精確函式簽章 | 看 header |
| enum、常數、錯誤碼 | grep code（會過期又能自動取得，見規則 5）|
| API 變更歷程 | `scripts/api-diff.js` 產生 |
| 法規原文 | 另有專門的法規知識庫 |
