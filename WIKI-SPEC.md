...
- [ ] 四行答題契約寫好
- [ ] 每條約束附 source 參考（檔案 → 函式名）
- [ ] 加進 `index.md` 對應分區（`raw/` 除外；子目錄不設目錄頁）
- [ ] 跑 `check-wikilinks.js` 與 `check-orphans.js`
- [ ] `log.md` append 一筆

### 改 code 之後

- [ ] 找出受影響的 wiki 頁（用 `sources` 反查）
- [ ] 更新內容 + `updated` + `source_commit`
- [ ] 若動到對外 API，跑 `scripts/api-diff.js` 確認增減
- [ ] `log.md` append 一筆

### 匯入素材

- [ ] 先講清楚這批是什麼、要放哪、屬於哪層，確認後才動手
- [ ] 放進 `raw/` 對應子目錄
- [ ] 自動加上素材警告 banner
- [ ] 更新 `raw/README.md`
- [ ] `log.md` append 一筆
